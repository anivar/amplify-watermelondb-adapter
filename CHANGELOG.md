...
### Fixed
- **Breaking compat fix**: rename `setup()` → `setUp()` to match the upstream
  `@aws-amplify/datastore` `Adapter` interface. DataStore calls
  `adapter.setUp(...)` during configure; the previous lowercase name silently
  failed.
- `save()` now returns `Promise<[T, OpType.INSERT | OpType.UPDATE][]>` (array of
  tuples) to match the upstream contract — was previously a single tuple.
- `batchSave()` now accepts `ModelInstanceMetadata[]` and returns
  `Promise<[T, OpType][]>`. Items with `_deleted: true` are now properly
  soft-deleted.

### Changed
- `WatermelonDBAdapter` now formally `implements StorageAdapter` from
  `@aws-amplify/datastore` so future drift is caught at compile time.
- `peerDependencies` tightened to `@aws-amplify/core ^6.16.0` and
  `@aws-amplify/datastore ^5.1.0` (drop dead `^5.0.0`/`^4.0.0` lower bounds).
- Drop `as any` cast on `storageAdapter` in `configureDataStoreWithWatermelonDB`.

## [1.2.2] - 2026-04-27

### Added
- Real LokiJS dispatcher integration test (`test/lokijs-integration.test.ts`) that boots WatermelonDB with the adapter under jest+jsdom, exercises `create`/`find`/`query`/`update`/`destroy`, and round-trips through IndexedDB via `fake-indexeddb`.
- `test/setup.ts` polyfills IndexedDB so the LokiJS persistence adapter actually runs in CI.
